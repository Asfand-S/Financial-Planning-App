...
function addProperty() {
    ...
    document.getElementById('propertiesContainer').appendChild(clone);
    propertyCount++;
    markFormChanged();
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('property-form').addEventListener('input', markFormChanged);
    document.getElementById('property-form').addEventListener('change', markFormChanged);
});

// Form submit handling
document.getElementById("property-form").addEventListener("submit", async function(e) {
    e.preventDefault();
    if (hasEmptyFields("#property-form")) {
    ...
    const propertyRadios = document.querySelectorAll('input[type="radio"]:checked');
    propertyRadios.forEach(radio => {
        formData.append("property_insured[]", radio.value);
    });

    try {
        const response = await fetch("/save_property", {
            method: "POST",
            body: formData
        });
