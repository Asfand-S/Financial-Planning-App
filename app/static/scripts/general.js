...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('general-form').addEventListener('input', markFormChanged);
    document.getElementById('general-form').addEventListener('change', markFormChanged);
});

// Form submit handling
document.getElementById("general-form").addEventListener("submit", async function(e) {
    e.preventDefault();
    if (hasEmptyFields("#general-form")) {
    ...
    try {
        const response = await fetch("/save_general", {
            method: "POST",
            body: formData
        });
